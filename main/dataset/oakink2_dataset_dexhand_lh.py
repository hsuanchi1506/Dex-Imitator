...
class OakInk2DatasetDexHandLH(ManipData):
    ...
    def __getitem__(self, index):
        ...
        mesh = Meshes(
            verts=torch.from_numpy(obj_mesh_trimesh.vertices[None, ...].astype(np.float32)),
            faces=torch.from_numpy(obj_mesh_trimesh.faces[None, ...].astype(np.int64)), 
        )

        rs_verts_obj = self.random_sampling_pc(mesh, save_img_path="debug_pc_lh.png")
        ...
        os.makedirs("debug_vis/left", exist_ok=True)
        ...
        for start in range(0, seq_len, 160):
            ...
            out_png = os.path.join(
                "debug_vis/left",
                f"relpos_lh_{index[0]}_stage{stage}_t{start:04d}_grid.png"
            )
