...
# from .base import ManipData, save_joint_pc_with_vectors
from .oakink2_dataset_utils import load_obj_map, as_mesh
from .decorators import register_manipdata
from typing import List, Optional

from .base import ManipData, draw_joint_pc_with_skeleton, save_joint_pc_with_skeleton_grid
...
class OakInk2DatasetDexHandRH(ManipData):
    ...
    def __getitem__(self, index):
        ...
        mesh = Meshes(
            verts=torch.from_numpy(obj_mesh_trimesh.vertices[None, ...].astype(np.float32)),
            faces=torch.from_numpy(obj_mesh_trimesh.faces[None, ...].astype(np.int64)),  # ← 要用 int64
        )

        rs_verts_obj = self.random_sampling_pc(mesh, save_img_path="debug_pc_rh.png")
        ...
        os.makedirs("debug_vis/right", exist_ok=True)

        xlim = (-0.1, 0.1)
        ylim = (0.0, 0.2)
        zlim = (0.0, 0.2)

        joint_names = data["joints_all_names"]
        seq_len = data["obj_verts_transf"].shape[0]

        # Save every 160 step [t, t+40, t+80, t+120]
        for start in range(0, seq_len, 160):
            idxs = [t for t in (start, start+40, start+80, start+120) if t < seq_len]
            frames = []
            for t in idxs:
                frames.append({
                    "points":      data["obj_verts_transf"][t],  # (N,3)
                    "joints":      data["joints_all"][t],        # (J,3)
                    "joint_names": joint_names,
                    "wrist":       data["wrist_pos"][t],         # (3,)
                    "obj_T":       data["obj_trajectory"][t],    # (4,4)
                    "title":       f"t = {t}"
                })

            out_png = os.path.join(
                "debug_vis/right",
                f"relpos_rh_{index[0]}_stage{stage}_t{start:04d}_grid.png"
            )
            save_joint_pc_with_skeleton_grid(
                frames, out_png, layout=(2, 2),
                xlim=(-0.1, 0.1), ylim=(0.0, 0.2), zlim=(0.0, 0.2),
                show_world_axes=True,
                contact_thresh=0.01,           # 5 cm
                contact_color="green",
            )
